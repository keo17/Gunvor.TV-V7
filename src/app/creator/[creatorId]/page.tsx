...
import { Twitter, Instagram, Linkedin, Youtube, Globe } from "lucide-react"; // Example social icons
...
export default async function CreatorPage({ params }: CreatorPageProps) {
  ...
  const creatorContent = allContentItems.filter(item => 
    item.creators?.some(c => c.id === creator.id) || // If content lists creator by ID
        ...
        <CardHeader className="bg-muted/30 p-6 md:p-8 border-b">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8">
            <Avatar className="h-32 w-32 md:h-40 md:w-40 border-4 border-background shadow-md">
              ...
              <AvatarFallback className="text-5xl">{creator.name.substring(0, 1)}</AvatarFallback>
            </Avatar>
            <div className="flex-1 text-center md:text-left">
              ...
              {creator.socialMediaLinks && creator.socialMediaLinks.length > 0 && (
                <div className="mt-4 flex justify-center md:justify-start space-x-3">
                  {creator.socialMediaLinks.map(link => (
